...
Borzsony::Borzsony(ostream & os, ParamHandler & params): Simulation(os,params)
{}
...
bool Borzsony::run()
{
	...
	*/
	...
	//setOutflowType(ofBottomLeft);
	   ...
	   double kFluvDouble = 0e-5;
	   double kTectDouble = 0e-3;				// set below too!
	   ...
	   mapattr(nSizeY,nSizeX,dx,0, mxcatch);
	   ...
	   mapattr(nSizeY,nSizeX,dx,2.0, n);
	   ...
	   //loadFromAsc("d:\\pcrinst\\mx1033000.asc",mx1,10);
	   //loadFromAsc("d:\\terRAIN\\test\\mx1020000.asc",mx1,10);
	   loadFromAsc("d:\\pcrinst\\borzsony50.asc",mx1,50);
	    loadFromAsc("d:\\pcrinst\\tst8.asc",mxcatch,50);
	   //printmx(mx3); 
	   //mx1 = mx1 + tst2;
	   mxEdges = mx1;
      ...
      //         }
			   ...
			   for ( j = 0; j < nSizeX; j++ ){
						for ( l = 0; l < nSizeY; l++ ){
							...
							if (mxcatch(l,j) == 0){
								mxAccflux(l,j) = 0(l,j); 
								mxSlopeCorrected(l,j) = 0(l,j); 
								mxLongest(l,j) = 0(l,j); 
								mxShortest(l,j) = 0(l,j); 
								elongation(l,j) = 0(l,j); 
								incision(l,j) = 0(l,j); 
								dAdLA(l,j) = 0(l,j); 
								dSdLS(l,j) = 0(l,j); 
								path1(l,j) = 0(l,j); 
								...
								fpelo(l,j) = 0(l,j); 
								fpLongest(l,j) = 0(l,j); 
								fpShortest(l,j) = 0(l,j); 
								fpdAdLA(l,j) = 0(l,j); 
								fpdSdLS(l,j) = 0(l,j); 
								fp2Accflux(l,j) = 0(l,j); 
								fp2Slope(l,j) = 0(l,j); 
								fp2elo(l,j) = 0(l,j); 
								fp2Longest(l,j) = 0(l,j); 
								fp2Shortest(l,j) = 0(l,j); 
								fp2dAdLA(l,j) = 0(l,j); 
								fp2dSdLS(l,j) = 0(l,j); 								  
							}
						}
...
//	
//	   // model size:
//	   size_t nSizeX = 80;  //797;
//	   size_t nSizeY = 80;  //788;
//	   size_t slopeBreak = 1;		// can not be zero!
//	   // statfile:
//	   StatFile statFile("statfile",15);
//	   StatFile statFile2("statfile2",15);
...
//	 
// 
//	   //loadFromAsc("d:\\pcrinst\\mx1015000.asc",mx1,10);
//	   //loadFromAsc("d:\\terRAIN\\test\\mx1020000.asc",mx1,10);
//	   //loadFromAsc("d:\\pcrinst\\borzsony50.asc",mx1,10);
//	   //printmx(mx3); 
//	   //mx1 = mx1 + tst2;
